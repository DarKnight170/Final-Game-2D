...
namespace YkinikY
{
    public class PlayerController_ykiniky : MonoBehaviour
    {
        ...
        void MovimentUpdate()
        {
            ...
            {
                transform.position += 1 * Time.deltaTime * velocity * Vector3.left;
                GetComponent<SpriteRenderer>().flipX = true;
            }
            if (Input.GetKey(KeyCode.RightArrow) || Input.GetKey(KeyCode.D))
            {
                transform.position += 1 * Time.deltaTime * velocity * Vector3.right;
            ...
            transform.position += 10 * Time.deltaTime * velocity * Vector3.right * Input.GetAxis("Horizontal");
            if (Input.GetAxis("Horizontal") == 11)
            {
                GetComponent<SpriteRenderer>().flipX = false;
            }
